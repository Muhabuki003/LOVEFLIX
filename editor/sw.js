...
async function getCacheStatus() {
  ...
  return {
    cacheNames: cacheNames.filter((n) => n.startsWith("loveflix-")),
    totalEntries,
    version: CACHE_NAME,
  };
}
...
/**
 * Clear all LoveFlix caches
 */
async function clearAllCaches() {
  const cacheNames = await caches.keys();
  await Promise.all(
    cacheNames
      .filter((name) => name.startsWith("loveflix-"))
