...
export const handleControls = () => {
  ...
  controls.listen('selected', (async ({ cell }: { cell: VoroforceCell }) => {
    if (cell) {
      ...
      setFilm(film)
      // LoveFlix: selecting a cell opens that video in the LoveFlix player.
      if (LOVEFLIX_ENABLED && film?.loveflix?.id) {
        window.location.href = `/player.html?id=${encodeURIComponent(film.loveflix.id)}`
      }
      // controls.pinPointer()
    } else {
